...
# Initialize pricing and distance calculators
...
# -----------------------------
# Payment & Fare Endpoints (PUBLIC key)
# -----------------------------
...
def fare_estimate(payload: FareEstimateIn, x_api_key: str | None = Header(default=None, alias="X-API-Key")):
    ...
    try:
        with db_conn() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    (
                        str(quote_id),
                        str(payload.ride_id) if payload.ride_id else None,
                        "USD",
                        float(estimated_distance_miles),
                        float(estimated_duration_min),
        ...
        raise HTTPException(status_code=500, detail=f"Failed to create fare quote: {e}")
